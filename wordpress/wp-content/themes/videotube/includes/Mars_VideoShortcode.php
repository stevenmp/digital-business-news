...
if( !class_exists('Mars_VideoShortcode') ){
	class Mars_VideoShortcode {
		...
		function myoutube( $attr, $content ){
			...
			$frame = '<iframe class="frmvideo" width="'.$width.'" height="'.$height.'" src="//www.youtube.com/embed/'.$video_id.'?autoplay=1" frameborder="0" allowfullscreen></iframe>';
		...
		function mvimeo( $attr, $content ){
			...
			$height = isset( $attr['height'] ) ? $attr['height'] : 300;			
			$video_id = $attr['video_id'];
			$frame = '<iframe class="frmvideo" src="//player.vimeo.com/video/'.$video_id.'?portrait=0&autoplay=1" width="'.$width.'" height="'.$height.'" frameborder="0" allowfullscreen></iframe>';
			return do_shortcode($frame);
		}
