...
if( !class_exists( 'Mars_LoginRegister_Template' ) ){
	class Mars_LoginRegister_Template {
		...
		function login_form_register(){
			...
			if( $loginpage ){
				if( get_option('permalink_structure') ){
					wp_redirect( get_permalink( $loginpage ) . '?action=register' );	
				}
				else{
					wp_redirect( get_permalink( $loginpage ) . '&action=register' );
				}
		...
		function login_form_bottom(){
			return '<input type="hidden" name="action" value="vt_ajax_login"><input type="hidden" name="button_label" value="'.__( 'Log In','mars' ).'">';
		}
		...
		function vt_ajax_register(){
			...
			echo json_encode( array(
				...
				'redirect_to'	=>	apply_filters('vt_registered_redirect_to', null)	
			) );
			exit;
		...
		function vt_ajax_login(){
			...
			if ( is_wp_error($user) ){
				...
				echo json_encode( array(
					...
					'redirect_to'	=>	apply_filters('vt_logged_redirect_to', null)
				) );
				exit;
			}
		}
